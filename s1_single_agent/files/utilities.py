...
class Utilities:
    ...
    def shared_files_path(self) -> Path:
        ...
        return Path(__file__).parent.parent.resolve() / "files"

    def load_instructions(self, instructions_file: str) -> str:
        """Load instructions from a file."""
        file_path = Path(os.path.join(self.shared_files_path, instructions_file.lstrip('/')))
    ...
    async def create_vector_store(
        ...
        # Upload the files
        for file in files:            
            file_path = os.path.join(prefix, file.lstrip('/'))
